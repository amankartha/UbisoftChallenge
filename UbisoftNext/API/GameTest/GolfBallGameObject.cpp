...
GolfBallGameObject::GolfBallGameObject(Game* instance, int id, int player_index,int playerid) : GameObject(instance,id) , m_player_index(player_index) , m_player_id(playerid)
{
}
...
void GolfBallGameObject::Init()
{
	...
	AddComponent<Crigidbody>(GameInstance->GetPhysicsSimulation(), 25.0f, 0.3f,0.8f,false,false);
